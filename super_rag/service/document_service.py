...
from super_rag.index.manager import document_index_manager
from super_rag.objectstore.base import get_async_object_store
from super_rag.schema import view_models
from super_rag.schema.view_models import Chunk, DocumentList, DocumentPreview, VisionChunk
from super_rag.utils.pagination import (
    ListParams,
    PaginatedResponse,
    PaginationHelper,
    PaginationParams,
...
from super_rag.utils.uncompress import SUPPORTED_COMPRESSED_EXTENSIONS
from super_rag.utils.utils import calculate_file_hash, generate_vector_db_collection_name, utc_now
...
def _trigger_index_reconciliation():
    ...
    try:
        ...
        from config.ray_tasks import reconcile_indexes_task
...
class DocumentService:
    ...
    async def _check_document_quotas(self, session: AsyncSession, user: str, collection_id: str, count: int):
        ...
        # from super_rag.db.models import UserQuota
    ...
    def _get_index_types_for_collection(self, collection_config: dict) -> list:
        ...
        """
        index_types = [
            db_models.DocumentIndexType.VECTOR_AND_FULLTEXT,
        ]
    ...
    async def get_document_chunks(self, user_id: str, collection_id: str, document_id: str) -> List[Chunk]:
        ...
        async def _get_document_chunks(session):
            ...
            stmt = select(db_models.DocumentIndex).filter(
                ...
                db_models.DocumentIndex.index_type == db_models.DocumentIndexType.VECTOR_AND_FULLTEXT,
            )
            result = await session.execute(stmt)
