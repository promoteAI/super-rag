...
@router.get("/agents")
async def list_agents_view(request: Request, user: User = Depends(required_user)) -> view_models.AgentList:
    return await agent_service.list_agents(str(user.id))


@router.get("/agents/{agent_id}")
async def get_agent_view(request: Request, agent_id: str, user: User = Depends(required_user)) -> view_models.AgentRecord:
...
async def delete_agent_view(request: Request, agent_id: str, user: User = Depends(required_user)):
